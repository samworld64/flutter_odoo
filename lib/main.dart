...
void main() async {
  ...
  try {
    ...
    final session = await client.authenticate(dotenv.env['ERP_DB']!,
        dotenv.env['ERP_USER']!, dotenv.env['ERP_PASSWORD']!);
    print(session);
    print('Authenticated');
    ...
    if (res.length == 1) {
      ...
      final user_avatar =
          '${dotenv.env['ERP_URL']!}/web/image?model=res.user&field=$image_field&id=$uid&unique=$unique';
